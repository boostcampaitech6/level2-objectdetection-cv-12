...
def get_bboxes(annotation, data_loader, model):
    ...
    for batch_idx, image_batch in enumerate(tqdm(data_loader)):
        ...
        for idx in range(batch_size):
            ...
            pred_info = predictions.pandas().xyxy[idx]
            classes = pred_info['class'].tolist()
            bboxes = pred_info[['xmin', 'ymin', 'xmax', 'ymax']].values.tolist()
            conf = pred_info['confidence'].tolist()
            # submission 파일에 저장될 내용
            prediction_string = ''
...
def inference():
    # model 생성
    model = torch.hub.load('/data/ephemeral/home/yolov5', 'custom', path=LOAD_MODEL_FILE, source='local')
    ...
    prediction_strings, file_names = get_bboxes(
        annotation, test_data_loader, model
    )
    submission = pd.DataFrame()
    submission['PredictionString'] = prediction_strings
    submission['image_id'] = file_names
    submission.to_csv('./exp1.csv', index=None)
